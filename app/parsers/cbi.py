"""
app/parsers/cbi.py – Central Bank of India statement parser.

CBI column layout:
  0: Date  1: Particulars  2: Chq/Ref No.  3: Debit  4: Credit  5: Balance
...
from app.parsers.base import (
    BankParser, parse_amount, parse_date, pick_debit_credit,
)

_SKIP_RE = re.compile(
    r"\b(opening\s+bal|closing\s+bal|balance\s+b/?f|total|brought\s+forward)\b",
    re.IGNORECASE,
)
...
_BAL_SUBS    = ("balance",)
_DESC_SUBS   = ("particulars", "description", "narration", "remarks")
_DATE_SUBS   = ("date",)
_REF_SUBS    = ("chq", "ref no", "cheque", "reference")
...
def _desc_type(desc: str) -> Optional[str]:
    if _DR_RE.search(desc): return "debit"
    if _CR_RE.search(desc): return "credit"
    return None


class CBIParser(BankParser):
    BANK_NAME = BankName.cbi
        ...
        for row in table:
            ...
            if has_debit and has_credit:
                ...
                for i, cell in enumerate(cells):
                    ...
                    elif _matches(cell, _DATE_SUBS)  and self._date_col   is None: self._date_col = i
                break
    ...
    def _parse_row(self, row: list[str]) -> Optional[TransactionCreate]:
        ...
        if self._debit_col is not None and self._credit_col is not None:
            ...
            desc_idx    = self._desc_col   if self._desc_col   is not None else 1
            ref_idx     = self._ref_col    if self._ref_col    is not None else 2
            ...
            col3 = self._safe_cell(row, 3); col4 = self._safe_cell(row, 4)
            balance_raw = self._safe_cell(row, 5)
            t = _desc_type(desc)
            a = parse_amount(col3); b = parse_amount(col4)
            if t == "debit":   debit_raw, credit_raw = (col3 if a else col4), ""
            elif t == "credit": debit_raw, credit_raw = "", (col4 if b else col3)
            else: debit_raw, credit_raw = pick_debit_credit(col3, col4)
        elif n == 5:
            date_str = self._safe_cell(row, 0); desc = self._safe_cell(row, 1)
            ref_no = self._safe_cell(row, 2); balance_raw = self._safe_cell(row, 4)
            col3 = self._safe_cell(row, 3)
            debit_raw = col3; credit_raw = ""
        else:
            return None
